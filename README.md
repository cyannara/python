...
## 컴프리헨션(Comprehension)
파이썬의 컴프리헨션(Comprehension)은 리스트, 셋, 딕셔너리 등 자료구조 내에서 반복문과 조건문을 한 줄로 작성하여 코드를 간결하고 효율적으로 만드는 파이썬 고유의 문법
```python
리스트 컴프리헨션 (List Comprehension)
#대괄호 [] 안에 표현식과 for문을 사용하여 리스트를 생성합니다.
#예시: [x * 2 for x in range(5)] 
# [0, 2, 4, 6, 8]
```
```
딕셔너리 컴프리헨션 (Dict Comprehension)
중괄호 {} 안에 key: value 형태와 for문을 사용합니다.
예시: {x: x**2 for x in range(3)} 
 {0: 0, 1: 1, 2: 4}
```
```
셋 컴프리헨션 (Set Comprehension)
중괄호 {} 안에 for문을 사용하여 중복을 허용하지 않는 집합을 생성합니다.
예시: {x for x in 'apple'} 
 {'a', 'p', 'l', 'e'}
```
## 실기 대비
